...
export function activeSwipe(message: TavernMessage): string {
  return String(
    message.swipe_id ??
      message.swipeId ??
      (Array.isArray(message.swipes) ? message.swipes.length - 1 : '0'),
  );
}
export async function fingerprintMessages(
  chat: TavernMessage[],
): Promise<MessageFingerprint[]> {
  return Promise.all(
    chat.map(async (message, ordinal) => {
      ...
      const swipeId = activeSwipe(message);
      return {
        fingerprint: await sha256(
          JSON.stringify([role, speaker, String(message.mes ?? ''), swipeId]),
        ),
        role,
        speaker,
        swipeId,
...
export async function chatPrefixFingerprint(
