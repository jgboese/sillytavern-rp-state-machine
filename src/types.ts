...
export type Status =
  'ready' | 'extracting' | 'replaying' | 'needs-reseed' | 'error';
...
export interface GameState {
  ...
  inventory: Record<
    string,
    Required<Pick<Entity, 'id' | 'name' | 'quantity'>> & { note?: string }
  >;
...
export interface ReviewItem {
  id: string;
  kind: 'extraction' | 'semantic' | 'manual' | 'replay';
  message: string;
  raw?: string;
