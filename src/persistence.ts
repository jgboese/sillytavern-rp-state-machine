...
export function emptyState(localDateTime: string, location: string): GameState {
  return {
    world: { localDateTime, location },
    player: { conditions: [] },
    currencies: {},
    inventory: {},
    characters: {},
...
export function seedContainer(
  ...
  return {
    schemaVersion: 1,
    status: 'ready',
    seed: { baseState, chatPrefixFingerprint: prefix, messageCount },
...
}): RpStateContainer | undefined {
  const value = context.chatMetadata[METADATA_KEY];
  const parsed = containerSchema.safeParse(value);
  return parsed.success ? (parsed.data as RpStateContainer) : undefined;
}
export function putContainer(
  context: { chatMetadata: Record<string, unknown> },
  container: RpStateContainer,
) {
  context.chatMetadata[METADATA_KEY] = container;
}
