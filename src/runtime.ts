...
export interface TavernContext {
  ...
  eventSource: {
    on(type: string, fn: (...args: unknown[]) => unknown): unknown;
    off?: (type: string, fn: (...args: unknown[]) => unknown) => unknown;
    removeListener?: (
      type: string,
  ...
  SlashCommandParser?: { addCommandObject(command: unknown): unknown };
...
function review(
  ...
  item: Omit<ReviewItem, 'id' | 'createdAt'>,
) {
...
function sameChat(ctx: TavernContext, chatId: string | undefined) {
  return ctx.chatId === chatId;
}
export class RpStateMachine {
  ...
  enqueue(work: () => Promise<void>) {
    ...
    this.queue = result.catch((error) =>
      console.error('[RP State Machine]', error),
    );
    return result;
  }
  start() {
    const ctx = this.getContext();
    for (const key of [...messageEvents, ...lifecycleEvents]) {
      const type = ctx.eventTypes[key];
      if (!type) continue;
      const handler = () => this.handle(key);
      ctx.eventSource.on(type, handler);
      this.unsubs.push(() => {
        if (ctx.eventSource.off) ctx.eventSource.off(type, handler);
        else ctx.eventSource.removeListener?.(type, handler);
      });
    }
    const props = {
      name: 'rpstate',
      helpString: 'Open RP State Machine',
      unnamedArgumentList: [
        {
          name: 'action',
          enumList: ['pause', 'resume', 'retry', 'replay'],
          isRequired: false,
        },
      ],
      callback: (_args: unknown, value: string) => {
        void this.command(String(value ?? ''));
        return '';
      },
    };
    ctx.SlashCommandParser?.addCommandObject(
      ctx.SlashCommand?.fromProps ? ctx.SlashCommand.fromProps(props) : props,
    );
  }
  stop() {
    this.disposed = true;
    ...
    try {
      this.getContext().setExtensionPrompt?.(PROMPT_KEY, '', 1, 0, false, 0);
    } catch {
      /* chat may be gone */
    }
  }
  private handle(event: string) {
    ...
    if (event === 'GENERATION_ENDED') {
      this.generating = false;
      void this.enqueue(() => this.reconcile());
    }
  }
  private scheduleReconcile() {
    if (this.generating || this.reconcileTimer) return;
  ...
  async inject(generationType?: unknown) {
    const ctx = this.getContext(),
      chatId = ctx.chatId,
      metadataRef = ctx.chatMetadata.rp_state_machine,
      container = getContainer(ctx),
      prefs = preferences(ctx);
    const quietOrImpersonation = /quiet|impersonat/i.test(
    ...
    if (
      ...
      prefs.paused ||
      !prefs.injectionEnabled ||
      quietOrImpersonation
    ) {
      ctx.setExtensionPrompt?.(PROMPT_KEY, '', 1, 0, false, 0);
      return;
    }
    const recentIds = container.transactions
      .slice()
      .reverse()
      .flatMap((t) =>
        t.events.flatMap((e) =>
    ...
    const text = await renderPrompt(
      ...
      async (x) =>
        ctx.getTokenCountAsync
          ? ctx.getTokenCountAsync(x)
          : Math.ceil(x.length / 4),
      recentIds,
    );
    if (
      !sameChat(this.getContext(), chatId) ||
      this.getContext().chatMetadata.rp_state_machine !== metadataRef
    ) {
      this.getContext().setExtensionPrompt?.(PROMPT_KEY, '', 1, 0, false, 0);
      return;
    }
  ...
  private firstChanged(
    ...
    next: RpStateContainer['messages'],
    minimum: number,
    transactions: StateTransaction[],
  ) {
    if (!old.length) return minimum;
    const length = Math.max(old.length, next.length);
    const changed = Array.from(
      { length: Math.max(0, length - minimum) },
      (_, offset) => minimum + offset,
    ).find((i) => old[i]?.fingerprint !== next[i]?.fingerprint);
    if (changed !== undefined) return changed;
    return (
      Array.from(
        { length: Math.max(0, next.length - minimum) },
        (_, offset) => minimum + offset,
      ).find(
        (i) =>
          next[i].isAssistant &&
          !transactions.some(
            (t) =>
              t.sourceFingerprint === next[i].fingerprint &&
              (t.origin === 'automatic' ||
                t.explanation === 'Reviewed manual correction'),
          ),
      ) ?? -1
    );
  }
  async seed(state: GameState) {
    const intended = this.getContext().chatId;
    await this.enqueue(async () => {
      const valid = gameStateSchema.safeParse(state);
      if (!valid.success)
        throw new Error(`Invalid seed: ${valid.error.issues[0]?.message}`);
      const ctx = this.getContext();
      if (!sameChat(ctx, intended))
        throw new Error('Chat changed while seeding');
      const fingerprints = await fingerprintMessages(ctx.chat);
      const prefix = await chatPrefixFingerprint(
        fingerprints,
        fingerprints.length,
      );
      if (!sameChat(this.getContext(), intended))
        throw new Error('Chat changed while seeding');
      const container: RpStateContainer = {
        schemaVersion: 1,
        status: 'ready',
        seed: {
          baseState: structuredClone(valid.data),
          chatPrefixFingerprint: prefix,
          messageCount: fingerprints.length,
        },
        currentState: structuredClone(valid.data),
  ...
  private seedAnchor(container: RpStateContainer) {
    return `seed:${container.seed.chatPrefixFingerprint}`;
  }
  async manual(
    events: StateTransaction['events'],
    explanation: string,
    anchor?: string,
  ) {
    const intended = this.getContext().chatId;
    await this.enqueue(async () => {
      const ctx = this.getContext(),
        container = getContainer(ctx);
      if (!container || !sameChat(ctx, intended))
        throw new Error('Seed this chat first');
      const source = anchor
        ? container.messages.find((x) => x.fingerprint === anchor)
        : container.messages[container.messages.length - 1];
      const isPostSeed =
        !!source && source.ordinal >= container.seed.messageCount;
      const transaction: StateTransaction = {
        id: crypto.randomUUID(),
        origin: 'manual',
        sourceFingerprint: isPostSeed
          ? source!.fingerprint
          : this.seedAnchor(container),
        sourceOrdinal: isPostSeed ? source!.ordinal : undefined,
        sourceSwipeId: isPostSeed ? source!.swipeId : undefined,
        events,
        timestamp: new Date().toISOString(),
        explanation,
      };
      try {
        container.currentState = applyTransaction(
          container.currentState,
          events,
        );
      ...
      if (!sameChat(this.getContext(), intended))
        throw new Error('Chat changed during manual edit');
      putContainer(ctx, container);
      await ctx.saveMetadata();
      this.onChange();
    });
  }
  async discardReview(id: string) {
    const intended = this.getContext().chatId;
    await this.enqueue(async () => {
      const ctx = this.getContext(),
        container = getContainer(ctx);
      if (!container || !sameChat(ctx, intended)) return;
      container.reviewQueue = container.reviewQueue.filter((x) => x.id !== id);
      putContainer(ctx, container);
      await ctx.saveMetadata();
      this.onChange();
    });
  }
  async applyReviewedEvents(id: string, events: StateTransaction['events']) {
    const intended = this.getContext().chatId;
    await this.enqueue(async () => {
      const ctx = this.getContext(),
        container = getContainer(ctx);
      ...
      const item = container.reviewQueue.find((x) => x.id === id);
      if (!item) throw new Error('Review item no longer exists');
      const source = item.sourceFingerprint;
      const anchor = container.messages.find((x) => x.fingerprint === source);
      const transaction: StateTransaction = {
        id: crypto.randomUUID(),
        origin: 'manual',
        sourceFingerprint: anchor
          ? anchor.fingerprint
          : this.seedAnchor(container),
        sourceOrdinal: anchor?.ordinal,
        sourceSwipeId: anchor?.swipeId,
        events,
        timestamp: new Date().toISOString(),
        explanation: 'Reviewed manual correction',
      };
      try {
        const ordered = [...container.transactions, transaction]
          .map((value, index) => ({ value, index }))
          .sort(
            (a, b) =>
              (a.value.sourceOrdinal ?? -1) - (b.value.sourceOrdinal ?? -1) ||
              a.index - b.index,
          )
          .map((x) => x.value);
        const rebuilt = ordered.reduce(
          (state, entry) => applyTransaction(state, entry.events),
          structuredClone(container.seed.baseState),
        );
        container.currentState = rebuilt;
        container.transactions = ordered;
        container.reviewQueue = container.reviewQueue.filter(
          (x) => x.id !== id,
        );
      } catch (error) {
        item.kind = 'manual';
        item.message = `Reviewed events rejected: ${String(error)}`;
  ...
  async importState(value: unknown) {
    const intended = this.getContext().chatId;
    await this.enqueue(async () => {
      ...
      const ctx = this.getContext();
      if (!sameChat(ctx, intended))
        throw new Error('Chat changed during import');
      this.revision++;
      putContainer(ctx, parsed.data as RpStateContainer);
      await ctx.saveMetadata();
      this.onChange();
    });
  }
  async resetState() {
    ...
    await this.enqueue(async () => {
      const ctx = this.getContext();
      if (!sameChat(ctx, intended))
        throw new Error('Chat changed during reset');
      this.revision++;
      delete ctx.chatMetadata.rp_state_machine;
      await ctx.saveMetadata();
      this.onChange();
    });
  }
  async retry() {
    ...
    await this.enqueue(async () => {
      if (!sameChat(this.getContext(), intended))
        throw new Error('Chat changed before retry');
  ...
  async command(value: string) {
    ...
    if (command === 'pause' || command === 'resume') {
      const ctx = this.getContext();
      const p = preferences(ctx);
      p.paused = command === 'pause';
      ctx.extensionSettings.rp_state_machine = p;
      ctx.saveSettingsDebounced?.();
      this.onChange();
      return;
    }
    if (command === 'retry' || command === 'replay') return this.retry();
    document.querySelector<HTMLButtonElement>('[data-rpstate-open]')?.click();
  }
}
